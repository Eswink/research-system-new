...
class PreflightFindingCode(StrEnum):
    ...
    - ENDPOINT_URL_DENIED：出网 URL 策略面（GOAL-007 EC-02 门链第一环；被拒时同 endpoint
      上不再派生 ENDPOINT_UNHEALTHY——策略没放行就没有可谈的健康事实）
    - MODEL_MISSING / MODEL_ELIGIBILITY / TOOL_UNAVAILABLE / SUPPLY_CHAIN_UNPINNED：
      模型/工具面（与 CompileFindingCode 同名的运行时复检）
    - TOOL_RISK_ELEVATED：工具 effect/risk 分层 HIGH/CRITICAL 提示（M8）
    - TOOL_HEALTH_UNPROVEN / TOOL_HEALTH_DEGRADED：provider 健康三态（WP-D；
      探测不可运行/未声明 health_check = UNKNOWN 只警示不阻断，不伪装健康）
    ...
    - SKILL_MISSING / SKILL_DIGEST_MISMATCH / SKILL_DEPRECATED /
      SKILL_RETIRED：Skill Registry 面（M8）
    """
    ...
    ENDPOINT_URL_DENIED = "ENDPOINT_URL_DENIED"
    MODEL_MISSING = "MODEL_MISSING"
    MODEL_ELIGIBILITY = "MODEL_ELIGIBILITY"
    TOOL_UNAVAILABLE = "TOOL_UNAVAILABLE"
    SUPPLY_CHAIN_UNPINNED = "SUPPLY_CHAIN_UNPINNED"
    ...
    TOOL_RISK_ELEVATED = "TOOL_RISK_ELEVATED"
    TOOL_HEALTH_UNPROVEN = "TOOL_HEALTH_UNPROVEN"
    TOOL_HEALTH_DEGRADED = "TOOL_HEALTH_DEGRADED"
