...
export function TelemetryView({ telemetry }: { telemetry: RunTelemetryDto }) {
  return (
    <div className="operations-telemetry" data-testid="telemetry-view">
      ...
      <p>outbox pending: {telemetry.outbox.pending}</p>
      <p>
        telemetry sink: {telemetry.sink.enabled ? "on" : "off"}; dropped{" "}
        {String(telemetry.sink.dropped)}
      ...
      <ul>
        {cost.dimensions.map((dimension) => (
          <li key={`${dimension.dimension}/${dimension.resource_key}`}>
            ...
            {dimension.amount.minor_units ?? "n/a"}{" "}
            {dimension.amount.currency} ({dimension.entry_count} entries)
          </li>
        ))}
      </ul>
      <p>
        total: {cost.total.status}{" "}
        {cost.total.minor_units ?? "n/a"} {cost.total.currency}
      </p>
    </div>
  );
}
        ...
        <div key={index} data-testid={`trend-segment-${String(index)}`}>
          {segment.points.map((point) => (
            <p key={point.report_digest}>
              ...
              (pass {String(point.pass_count)}, fail {String(point.fail_count)},{" "}
              infra {String(point.infra_error_count)})
            </p>
          ))}
