...
| Research OS Python 服务镜像（`infra/docker/research/Dockerfile.python`）— id `research_os_python_service_image` | Python 3.12 slim `sha256:dd2937…` + uv 0.11.18 `sha256:78bc42…` 双 stage 固定；PSF-2.0 + Apache-2.0 OR MIT；Python 依赖仍由 `uv.lock` 冻结 | ADOPTED Compose API/Gateway/validation Worker runtime |
| Research Console 镜像（`infra/docker/research/Dockerfile.console`）— id `research_os_console_image` | Node 22.18.0 bookworm-slim `sha256:752ea8…`；Node/pnpm MIT，Debian 组件保留各自许可证；`pnpm-lock.yaml` frozen install | ADOPTED Compose Console runtime |
| Research OS PostgreSQL 镜像（`infra/docker/research/Dockerfile.postgres`）— id `research_os_postgres_image` | PostgreSQL 16.15 alpine `sha256:cf78e7…`；PostgreSQL License；命名卷承载 Canonical State | ADOPTED Compose canonical-state runtime |
