...
2. **控制面无调用方认证**（不只是「授权粒度粗」）。无 API key / bearer / session /
   ...
   > **实况更新（2026-09-26 / GOAL-019）**：本条的「无认证」**已在写面上改变**——
   > mutating 方法现在受 bearer token 保护（见 §6.7）。**读面仍无认证**，且
   > §6.2 第 3–5 条（能力级策略、无归属概念、路径 id 不判归属）**全部仍然成立**。
...
1. **不覆盖「控制面无认证」这一事实的处置**。6.2 第 2 条把它记成事实；**要不要**、**何时**
   ...
   > **实况更新（2026-09-26 / GOAL-019）**：**写面**已补最小认证 ⇒ 上面「即可**读写**
   > 全部对象」现在应读作「即可**读**全部对象、且需 token 才能写」。**读面**的
   > 这半句**仍然成立**；「整面尚无校验点」现在只适用于**读面与对象级授权**
   > （BOLA/BFLA 仍**未做**，见 §6.7）。
...
- **前置**：必须先有**主体模型**——没有 principal 就没有「越权」可言。这一步
  **不能**靠加测试解决，它落在 M18（或 D-12(a) 明文授权的等价子集）。
  > **实况更新（2026-09-26 / GOAL-019）**：该前置**已具备最小形态**——
  > `Principal` 域值对象 + 请求级主体归因已落地（§6.7）。但**前置满足 ≠ (a) 已做**：
  > 本 GOAL **明文不做** (a)，逐路由授权清单 / BOLA·BFLA 负面测试 **一条都没有**。
...
- 本节**不得**被引用为「已做威胁建模」「授权面已覆盖」或任何形式的安全结论。
  ...
  `docs/security/THREAT_MODEL.md` 第 6 节（D-12(b) 草案）；其中**写面**已于
  2026-09-26（GOAL-019）补最小认证，**读面与对象级授权（BOLA/BFLA）仍未覆盖**」**。
  **新口径不得被简写成「授权面已覆盖」**——那会把未覆盖的读面与对象级授权抹掉。

### 6.7 实况更新：Control Plane 写面认证（GOAL-019，2026-09-26）

本节记录 §6.2 / §6.3 / §6.5 在 GOAL-019 之后的**当前**实况。**§6.2 的编号项保持原样**
（它们记录的是 GOAL-016 当时的空白，**不是**错误），本节与就地标注给出变更后的口径。

**认证覆盖**：写面（POST / PATCH / PUT / DELETE）已认证；读面（GET / HEAD）未认证；多租户与 RBAC 未实现。

- **保护范围**：只保护写面，分类**复用** `_MUTATING_METHODS`
  （`services/api/middleware.py`）；读面一律放行；**无**路径白名单（`GET /health`
  因是 GET 自动豁免）。
- **凭据**：`RESEARCHOS_CONTROL_PLANE_TOKEN`（可选 `RESEARCHOS_CONTROL_PLANE_PRINCIPAL_ID`），
  只从环境变量读、`hmac.compare_digest` 常数时间比较、**不落盘 / 不进日志、事件、遥测**；
  留空 ⇒ **认证关闭 + 启动显式警告**；CI 与本仓 live 夹具不设该变量 ⇒ 默认链路不变。
- **主体**：`Principal`（`packages/domain/principal.py`，id + 类型，**无**租户/角色字段）
  经请求级上下文落到 canonical 的事件 `actor` 字段，读面 `GET /runs/{id}/events` 可见。
- **对 BOLA / BFLA 的影响（务必按此读）**：写面有了认证 ⇒ **匿名写**这一路径被关掉；
  但**对象级**问题**一个都没解决**——没有「哪个主体能碰哪个对象」的判定点，
  也没有逐路由授权清单与负面测试。⇒ **BFLA 的「功能级」面在写面上从"无校验点"
  变成"有认证、无授权"；BOLA 完全未动**。
- **仍未覆盖（与 §6.3 同口径，逐条保留）**：读面未认证 / 多租户与 RBAC 未实现 /
  BOLA·BFLA 专项未做 / 逐调用方身份未做（单一共享 token ⇒ 单一主体，且**不接受**
  调用方自报身份）/ 反代与 TLS 未验证 / 主体无数据库级约束 / `R-M1` 未收口。
- **零夸大**：本节**不是**安全结论。**不得**据此宣称「项目安全」或「授权面已覆盖」；
  可引用的口径见 §6.6。
