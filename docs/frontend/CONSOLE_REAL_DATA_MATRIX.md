...
| 15 | `ops/matrix` | gap | **保持** | 该页**按设计不消费任何读面**——呈现加载/空/错误/权限/未知等 UI 组件状态 | 「界面状态说明页（非实时运维状态）」；`DESIGN:not-a-live-ops-surface`（不做的原因：它是界面状态说明页，不是实时运维状态面；三处口径同源见 EC-04） | 这是**设计口径**而非待建功能：页面文案与文档同源，见 EC-04 的处置 |
