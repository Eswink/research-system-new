...
### 2.2 怎么验证 401 是否正常（四步，可照抄）

**为什么需要这一节**：认证开启后「写被拒」既可能是**认证生效**，也可能是**别的错**
（载荷非法、路径写错、服务没起来）。下面四步把两者分开——**判据是状态码与 `detail`**，
不是「看起来通了」。

前置：服务已用 `RESEARCHOS_CONTROL_PLANE_TOKEN` 启动（§2.1 的 A 或 B 形态之一）。
下面用 `<TOKEN>` 表示那个值；**命令里不要把值写进文件或 shell 历史**（用变量引用）。

```bash
# ① 读面不认证：不带 token 也必须 200
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:8000/health          # 期望 200

# ② 写面不带 token：必须 401，且 detail 点名「缺 Authorization: Bearer 头」
curl -s -X POST http://127.0.0.1:8000/projects \
  -H 'Content-Type: application/json' -H 'Idempotency-Key: verify-1' \
  -d '{"name":"verify-probe"}'                                                # 期望 401 + 点名缺头

# ③ 写面带错 token：必须 401，且 detail 点名「token 不匹配」
curl -s -X POST http://127.0.0.1:8000/projects \
  -H 'Content-Type: application/json' -H 'Idempotency-Key: verify-2' \
  -H 'Authorization: Bearer not-the-token' \
  -d '{"name":"verify-probe"}'                                                # 期望 401 + 点名不匹配

# ④ 写面带对 token：必须 2xx（认证放行）
curl -s -o /dev/null -w '%{http_code}\n' -X POST http://127.0.0.1:8000/projects \
  -H 'Content-Type: application/json' -H 'Idempotency-Key: verify-3' \
  -H "Authorization: Bearer $RESEARCHOS_CONTROL_PLANE_TOKEN" \
  -d '{"name":"verify-probe"}'                                                # 期望 201
```

**读法**（务必按此判，否则会把别的错读成认证生效）：

| 现象 | 说明 | 处置 |
| --- | --- | --- |
| ① 不是 200 | 读面被拦了 ⇒ 与口径不符 | 检查服务是否真的在跑、路径是否写对（**不是**认证问题） |
| ② / ③ **不是** 401 | 写面没被认证保护 | 确认进程真的读到了该变量（**必须重启**才生效，见 §3） |
| ④ **不是** 2xx | 认证放行了但请求被别的规则拒（如载荷非法 422） | 看 `detail`：认证已通过，改的是**请求**而不是认证 |
| ② / ③ 是 401 但 `detail` 与上表不符 | 响应形态漂移 | 如实登记；**不要把别的 4xx 读成 401** |

**可复核的实测记录**（GOAL-020 EC-03；脚本
`scratch/goal020-ec03-401-verification.py`，对真实控制面进程跑，token 现场生成且只走
环境变量、不落盘）：关闭态 `GET /health` **200** / `POST /projects` **201** /
启动警告在场；开启态 **200 / 401 / 401 / 201**，两个 401 的 `detail` 分别点名
「缺头」与「不匹配」。

### 2.3 部署面：开认证前必须知道的四条（**含本机不可验证部分**）

**为什么写这一节**：认证在**进程内**实现；一旦前面有反代、TLS 终止或多副本，
「谁在调用」这一层就多出几个**本仓没有验证面**的变量。以下四条是**部署前检查项**，
其中**第 3、4 条在本机无法验证**——按 GOAL-019 登记的口径**如实保留**。

1. **反代必须透传 `Authorization` 头**（**不得**剥离 / 覆盖）。若反代自己也用
   `Authorization` 做上游认证，两边会**互相顶替** ⇒ 让两个信任域不要共用同一个头。
2. **TLS 必须在反代终止**：token 是 bearer 凭据，**明文 HTTP 上等于公开**。
   本仓开发脚本绑 `127.0.0.1`、容器发布端口也是回环；**跨主机暴露前必须上 TLS**。
3. **多副本 = 每个副本都要同一个变量值**：本实现**没有**共享会话 / 密钥协调面，
   副本间不一致会让请求**随机成功或失败**（表现为间歇 401）。这条**未在本机验证**
   （本机只跑单进程）：登记为**未验证**，**不得**当作「已知可工作」。
4. **反代 / TLS / 多副本下的认证行为未验证**：本节只给**检查项**，**不产生**验证结论。
   要把它变成已验证需要真实部署拓扑（本机没有）——**不得**据本节宣称部署面已覆盖。

**可复核的边界（本仓已有的实测）**：控制面容器内 `uvicorn` 监听 `0.0.0.0`
而**发布端口是回环**；开发脚本绑 `127.0.0.1`。⇒ 不得把「开发时只在本机」读成
「只可能被本机访问」。
