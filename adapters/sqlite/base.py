"""SQLite adapter 公共基座：call recording + close 语义。

Contract suite 要求注册表内每个实现可审计（hasattr(instance, "calls")，
tests/contracts/test_common_contract.py）；close 后调用抛
PermanentPortError（resource cleanup 语义，PORTS.md）。
"""
...
class SqliteAdapterBase:
    """SQLite adapter 公共基座：call log 与 close 语义。"""
