...
| [EXP-20260827-001](entries/EXP-20260827-001.md) | ACTIVE | 0.5 | repository | 2026-11-25 | Read/Delete 工具读取或删除 `.cursor/runtime/**`（observations/distillation）被 fail-closed 拒绝（Read ×6 + Delete ×1 跨 4 会话，duration <4ms）；改用 Shell Get-Content / Remove-Item -Force 成功：提取 error_signature/清理标记时直接用 Shell，不重试 Read/Delete、不改 hook。该家族摘要含被拒路径故每次都变，结构上永不达跨会话 ≥2 摘要门槛，只能按机制识别 |
...
| [EXP-20260823-003](entries/EXP-20260823-003.md) | ACTIVE | 0.5 | repository | 2026-11-21 | CURSOR_ERROR 瞬时失败跨会话（Read d42fb89b…/Shell f3eb4221… 各 2 会话；8/26 新增 Shell 0384c5d8…，首个长时 23.8s 实例；8/27 新增 Read 523ede9c… 2.9ms；8/30 全量复核新增跨会话 Read f4323f3c…（2 会话）与 Shell eb953c57…（3 会话），家族 LEARN-20260823-001 已覆盖不重复提案）：重试一次或改替代路径，不当作命令失败；统计时过滤 tool_name=null 的 TOOL_FAILURE 空载荷哨兵（e3b0c442…=sha256("")）；跨会话以不同 observation 文件为准 |
