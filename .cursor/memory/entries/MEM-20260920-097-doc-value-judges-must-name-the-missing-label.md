...
**第三条（GOAL-009 cycle 5 补的，实测教训）——断言要落在「行」上，不能落在「整节」上。**
第一版判据只断言「§8 里出现过『快照』」，于是把**撤销那一行**改写成
「每次调用都读一次环境，所以立即生效」时，整节断言**仍然绿**（别处还有『快照』字样）。
改成 `assert "新构造" in _row(label)`（逐行）之后，同一次改写当即变红。
**一句话**：`_row(label)` 取出来的**那一行**才是被钉住的契约；
`_section()` 级别的「在场」断言只能证明「某个词在附近出现过」。
