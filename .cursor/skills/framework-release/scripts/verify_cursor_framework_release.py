...
def main() -> int:
    ...
    version=(ROOT/"VERSION").read_text(encoding="utf-8").strip(); requested=args.version or version
    if requested!=version:
        print("ERROR: requested version mismatch"); return 2
    mp=ROOT/"FRAMEWORK_MANIFEST.json"; ep=ROOT/".cursor/releases/RELEASE_EVIDENCE.json"
    if not mp.exists() or not ep.exists():
        print("ERROR: release manifest/evidence missing"); return 2
    m=json.loads(mp.read_text(encoding="utf-8")); errors=[]
    if m.get("status")!="VALIDATED": errors.append("release status is not VALIDATED")
    if m.get("framework_version")!=version: errors.append("framework version mismatch")
    if m.get("version_source")!="VERSION": errors.append("manifest version_source mismatch")
    if m.get("release_evidence_sha256")!=sha(ep): errors.append("release evidence digest mismatch")
    try:
        ...
        for item in commands:
            ...
            if cmd and (str(cmd[0]).startswith("/") or (len(str(cmd[0]))>2 and str(cmd[0])[1:3] in {":\\", ":/"})):
            ...
            if len(output_sha)!=64 or any(ch not in "0123456789abcdef" for ch in output_sha.lower()):
    ...
    if missing_from_manifest: errors.append(f"manifest missing file entries: {missing_from_manifest[:20]}")
    if extra_in_manifest: errors.append(f"manifest references non-release files: {extra_in_manifest[:20]}")

    for item in m.get("files") or []:
        p=ROOT/item["path"]
        if not p.exists(): errors.append(f"missing {item['path']}")
        elif sha(p)!=item["sha256"]: errors.append(f"digest mismatch {item['path']}")
        elif p.stat().st_size!=item.get("bytes"): errors.append(f"size mismatch {item['path']}")
    if errors:
