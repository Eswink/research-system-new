...
## 更正（2026-09-16，cycle 9 CI 红项处置）

**CI run 35115260874 的 `quality-ubuntu-latest` 判红**在 `python/tests`，红的就是本轮的
反证用例 `test_statement_level_serialization_still_misses_the_row`：

```text
AssertionError: 语句级串行的形态没有复现出对不上的读…：[[], [], []]
```

**定性**：不是被测代码的问题，也不是"门禁太严"，而是**反证本身不可移植**——
这是真实竞态，复现率随并行度变化：

| 平台 | 负载（12 线程 × 8 轮，3 次尝试） | 复现 |
| --- | --- | --- |
| 8+ 核 Windows（开发机） | 288 次往返 | 每次 10~20 处读错 |
| 2 vCPU ubuntu runner（CI） | 288 次往返 | **0 处** |

顺带把"确定性做法"也证伪了：**拿着游标不放、让另一线程写+提交后再取行**
（单线程可编排的时间窗交错）**不触发**——竞态需要两个线程**同时**在 sqlite3 的
C 调用里（GIL 已释放），不是简单的时间顺序问题（scratch/goal3-cycle10-deterministic-probe.py）。

**处置**（见 PLAN-20260915-072 的 cycle-9 红项段）：把 CI 门禁从"负载压出读错"换成
**结构判据**（确定性、任何机器成立）——产品连接的读结果在锁内取尽（`MaterializedRows`），
退回语句级串行时是裸 `sqlite3.Cursor`；负载型复现器保留为**记录**（上面的数字与脚本），
不再当门禁。本文件其余结论（含 W-1 的"live 压不出这一类"）不受影响，且被这次 CI 结果**加强**：
读侧竞态的可观测性完全取决于并发密度。
