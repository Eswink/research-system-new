...
## 驱动适配（会话 / cron / 客户端 goal 模式）

GOAL 的契约是**文件驱动、驱动无关**：驱动只决定"谁把循环推进一轮"，本文件决定
"做什么、是否完成、何时停"。三类驱动按同一入口协议接入：

- **会话驱动（默认）**：用户指令（如「执行 GOAL-… 下一 cycle」）进入；同一会话内
  可在授权与预算允许时连续跑多个 cycle；上下文截断/会话终止不影响可恢复性（一切
  状态在文件与工作树）。
- **定时自动化驱动**：用客户端的周期任务（cron automation）按间隔触发，任务指令
  固定为「读 `goals/<GOAL-file>`，按循环入口协议推进一个 cycle；GOAL 非 ACTIVE
  即直接结束」。无人值守跨会话由此实现；频率、启停归用户，循环语义仍归本文件。
- **客户端 goal/auto 模式（若存在）**：其"目标持续推进"语义与 objective/
  exit_criteria 一一对应——每轮触发等于进入一次入口协议；其自动继续的停止条件
  以本文件为准：`status != ACTIVE`、EC 全 PASS、`budget.max_cycles` 触顶或
  `no_progress_stop_cycles` 命中即视为目标结束/阻塞。**客户端模式自带的
  迭代/重试上限一律让位于 frontmatter 的 budget/fix_policy**（文件为权威，
  防止双层限额互相掩盖）。

驱动不适配任何东西：换驱动不换契约；同时只允许一个驱动持有 ACTIVE GOAL 的推进权
（进入 cycle 时在迭代日志声明 owner 行，第二驱动见 ACTIVE-cycle 未收口即等待，
不并发双写）。
