...
| 3 | PLAN-20260921-129（EC-03） | 本 cycle derive 提交、本 cycle WP1 提交（协议定案；SHA 随 cycle 收口台账补记） | **derive 与 WP1 都只读代码/配置，未发起任何真实调用**；治理 `validate.py` 绿；实测得 E-1…E-8 + 候选表 + 一条**决定性能力面事实** | **见回合汇报**（台账尾巴口径：本条自身的 run 不回写文件） | — | EC-03 执行中；**WP1 已定案：取 (B)「新增一份真实协议并登记」**——(A) 不是被否决，而是**它的两个前提本轮答不了**：**A1** 的 10 条非自述来源今天只有「声明输入」（**否决**：只有计数变真、没有检索发生）与「工具结果」（**无生产调用方** + live 工具**惰性** ⇒ 必须先做检索接线，即 EC-02 的残余 **W-7**）；**A2** 的 11 个 phase 在「phase 数 = 会话数」下等于 **11 次真实会话**，与「live 调用取最小必要」冲突。**本轮新增决定性实测**：`evidence.read` **未被 `policy.yaml` 授予**（`default_effect: DENY`）⇒ `sort_analysis_v1` 的 review phase 在真实 policy 下**连 preflight 都过不去**（它此前只在 `FakePolicyEvaluator` 下被跑过）⇒ **在册 6 份协议没有一份**同时满足「语义对真实执行体成立」与「合约本轮可达」。(B) 的**规格 + 四条护栏 + 代价 + 诚实边界**已写死在 PLAN-129 的「WP1 定案」节 | cycle 3 续：**WP2** 按规格落地——`real_research_task_v1.yaml`（**1 phase** / `domain_researcher` / `artifact.read`（**已授予**集内）/ 头部写明真实执行体语义与受控范围）+ 新合约 `real_research_deliverable`（**恰一个** `ARTIFACT_EXISTS` + `EVIDENCE_COVERAGE ≥ 1`）+ 声明输入 `input-brief:real_research_v1`（新输入文件 + `DECLARED_INPUTS` + 同源判据映射）+ **登记同源**（含 E-8 的基线/release asset 按配方处理）→ WP3 canonical 判据 + 反证（改回 demo ⇒ 红）→ WP4 真实 run（终态如实）→ WP5 门禁 + RECHECK + 收口 |
