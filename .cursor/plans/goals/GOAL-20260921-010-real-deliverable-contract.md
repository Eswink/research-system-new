...
| 3 | PLAN-20260921-129（EC-03） | `15be1eb`（derive + ALL_PLAN）、`8cbdd62`（WP1 定案）、`a448fc3`（WP2 落地）、`004f916`（WP3 判据）、`2edb204`（WP4 live 模块）、本 cycle 收口提交（WP5 + RECHECK-129 + EC-03 置 PASS；**见下方 CI 台账尾巴**） | **m0 全量 3 次全 `PASS: profile=m0; 23 deterministic checks`**（`a448fc3`：4278 passed/15 skipped；`004f916`：4282/15；`2edb204`：4283/16）+ 治理 `validate.py` 每次绿 + 产品路径 `compile` ⇒ `PASS`（唯一 finding 是 INFO `DAG_ORPHAN_PHASE`，单 phase 无后继，**如实**）| `15be1eb` ⇒ M0 35587567675 + CodeQL 35587567385；`8cbdd62` ⇒ M0 35589392128 + CodeQL 35589391669；`a448fc3` ⇒ M0 35593973258（6 job）+ CodeQL 35593972477（3 job）；`004f916` ⇒ M0 35597030860 + CodeQL 35597030000；`2edb204` ⇒ M0 35598711734（6 job）+ CodeQL 35598711199（3 job）；**全部 success（逐 run 逐 job 实查）**；本 cycle 收口提交的 run **见回合汇报**（台账尾巴口径） | **本 cycle 自己造成并修好的三条**：① WP2 首次 m0 红——新 schema 未登记 + `additionalProperties: true`（`framework/validate_bundle`）⇒ 按仓库**既有维护路径**补注册表**一行**（先例 `4156238`）并把 schema 自身改 `false`，**已按压**（删行 ⇒ 红）；② WP3 首次 m0 红——`python/typecheck`：`services.api.deps` **不显式导出** `ApiDeps` ⇒ 改从定义处 `services.api.composition` 导入；③ **工具面**：CI 轮询脚本只打印 `head_sha` 命中的**第一个** run（差点把「两个 workflow 都绿」写成只有一个绿）⇒ 改成遍历该 SHA 的**全部** run 并**重查** | **未削能力、未改任何既有合约、未改门禁校验逻辑、未放宽任何断言语义**；本 cycle 唯一触及受治理文件的改动 = `validate_bundle.py` 注册表**一行**（`git diff` 仅一行新增，已按压，`RECHECK-129` W-1 待人工过目）。**真实调用仅 1 次**（WP4），`RESEARCHOS_AGENT_RUNTIME` 只作单条命令内联前缀、**未**写入 `.env` | EC-03 **PASS**（判据在离线三面钉死 + 一次真实 run 两侧都成立，样本 `f9bef830-0ee3-4f05-8d40-ca57c1f5e643`）。**下一步**：EC-04（运行时指纹四要素**落读面** + 「模型不存在」provider 侧真实样本**或**如实登记未实测）→ EC-05（「默认门离线」的**结构**判据，不依赖人工观察）→ EC-06（独立复检 + 干净 checkout 封印 + 残余登记） |
...
| cycle 3 派生（PLAN-129 + ALL_PLAN 投影） | `15be1eb` | [35587567675](https://github.com/Eswink/research-system-new/actions/runs/35587567675) | 六 job 全 **success**（`collector-quality` / `console-frontend` / `container-quality` / `eval-gate` / `quality-ubuntu-latest` / `quality-windows-latest`；terminal `status=completed conclusion=success`，逐 job 实查）；**同一次推送另触发 CodeQL** [35587567385](https://github.com/Eswink/research-system-new/actions/runs/35587567385) = success（3/3） |
| cycle 3 WP1（EC-03 协议定案；只读，未发调用） | `8cbdd62` | [35589392128](https://github.com/Eswink/research-system-new/actions/runs/35589392128) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35589391669](https://github.com/Eswink/research-system-new/actions/runs/35589391669) = success（3/3） |
| cycle 3 WP2（协议 + 合约 + 声明输入 + 登记面落地） | `a448fc3` | [35593973258](https://github.com/Eswink/research-system-new/actions/runs/35593973258) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35593972477](https://github.com/Eswink/research-system-new/actions/runs/35593972477) = success（3/3） |
| cycle 3 WP3（canonical 协议身份判据 + 常驻反证） | `004f916` | [35597030860](https://github.com/Eswink/research-system-new/actions/runs/35597030860) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35597030000](https://github.com/Eswink/research-system-new/actions/runs/35597030000) = success（3/3） |
| cycle 3 WP4（真实协议 live 模块 + **1 次**真实 run） | `2edb204` | [35598711734](https://github.com/Eswink/research-system-new/actions/runs/35598711734) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35598711199](https://github.com/Eswink/research-system-new/actions/runs/35598711199) = success（3/3） |
| cycle 3 收口（WP5 + RECHECK-129 + EC-03 置 PASS） | `7d588b9` | [35600813559](https://github.com/Eswink/research-system-new/actions/runs/35600813559) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35600813116](https://github.com/Eswink/research-system-new/actions/runs/35600813116) = success（3/3） |
| cycle 4 派生（PLAN-130 + ALL_PLAN 投影） | `89cfae8` | [35604253831](https://github.com/Eswink/research-system-new/actions/runs/35604253831) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35604253122](https://github.com/Eswink/research-system-new/actions/runs/35604253122) = success（3/3） |
| cycle 4 derive 证据补全 | `c9b0ebb` | [35606257297](https://github.com/Eswink/research-system-new/actions/runs/35606257297) | **六 job 结论 = cancelled**（`quality-ubuntu-latest` / `console-frontend` / `quality-windows-latest` 被**下一次推送**的 `cancel-in-progress` 取消；`collector-quality` / `container-quality` = success）——**如实登记为未完成**，该提交的验证由下一行（同为 cycle 4、且包含它的树）承担；**CodeQL** [35606257049](https://github.com/Eswink/research-system-new/actions/runs/35606257049) = success（3/3） |
| cycle 4 WP1（EC-04 定案；只读，未发调用） | `6e3ddba` | [35606713362](https://github.com/Eswink/research-system-new/actions/runs/35606713362) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35606712973](https://github.com/Eswink/research-system-new/actions/runs/35606712973) = success（3/3） |
| cycle 4 WP2（指纹四要素落读面 + 快照/类型/文档同源） | `c4a6f7c` | [35614561941](https://github.com/Eswink/research-system-new/actions/runs/35614561941) | 六 job 全 **success**（`collector-quality` / `console-frontend` / `container-quality` / `eval-gate` / `quality-ubuntu-latest` / `quality-windows-latest`，逐 job 实查）；**CodeQL** [35614561942](https://github.com/Eswink/research-system-new/actions/runs/35614561942) = success（3/3） |
| cycle 5 派生（PLAN-131 + ALL_PLAN 投影） | `99783e4` | [35626335166](https://github.com/Eswink/research-system-new/actions/runs/35626335166) | 六 job 全 **success**（逐 job 实查）；**CodeQL** [35626334361](https://github.com/Eswink/research-system-new/actions/runs/35626334361) = success（3/3） |
| cycle 5 交付（判据本体 WP2 + 测量 WP3 + RECHECK-131 + EC-05 置 PASS） | `87208aa` | [35635457030](https://github.com/Eswink/research-system-new/actions/runs/35635457030) | **`quality-windows-latest` = failure**（`egress guard: FAIL — the default gate attempted 31 non-loopback destination(s)`，31 条全部 `57.150.192.193:443`、全部归因到各测试模块 `import openhands.sdk` 行；末行 `FAILED: 1 check(s): python/tests=1`）；`quality-ubuntu-latest` / `console-frontend` / `eval-gate` / `container-quality` / `collector-quality` = **success**；**CodeQL** [35635454082](https://github.com/Eswink/research-system-new/actions/runs/35635454082) = success（3/3）。**如实登记为红**：根因 = 干净安装下 litellm 首次导入下载 `tiktoken` 词表（本机缓存已热 ⇒ 看不见）；**未**改判据凑绿，修法见 GOAL 迭代日志第 5 行⑦与 `RECHECK-20260922-131` 的 CI 回灌一节 |
...
- 2026-09-21 cycle 4 **收口**（EC-04，子 PLAN `PLAN-20260921-130`；RECHECK = `RECHECK-20260921-130`）：
  **四要素从「只活在测试的 `tmp_path`」变成产品路径上的 canonical 事实**——观测取自 adapter
  **已经在读**的 `ConversationStats`（**零额外调用**；SDK 哨兵 `"default"` 按缺项处理）→ run 收敛后
  落 `model.probed`（此前只声明、从未发出；**没有观测 ⇒ 不发事件**）→ `GET /runs/{id}` 的
  `execution.runtime_fingerprint` 合并呈现（`source` 标明占位还是实测、缺项逐项点名、
  冻结 payload **不被改写**）。**provider 侧样本已实测**（AC-4 第一条）：`agnes-anthropic` +
  一个不存在的标识 ⇒ 连通性通过、那次 chat 被 **5xx** 拒且**错误正文点名**该标识、返回 model 名
  `null` ⇒ **零回退、无静默映射**。**实测发现的新边界**（已写进 RUNBOOK §7 第 4 条）：
  「模型不存在」**没有专属失败类别**——与「中转站故障」共用 `MODEL_RELAY_UNAVAILABLE` 且该类别
  **可重试** ⇒ 「点名模型标识」是唯一区分读数面。**按压抓出并修好一条判据缺陷**（文本子串判据
  与「skip 不是红」）。**真实调用 3 次**（**超出最小必要，已登记 W-4**：第 3 次是为区分
  「谁拒的」而**必要**的步骤轨迹）。EC-04 **PASS**；EC-05/EC-06 仍 PENDING。
...
- 2026-09-21 cycle 3 WP1 **定案**（`driver=client-goal / owner=root-agent`）：**取 (B)「新增一份
  真实协议并登记」**。本轮**只读代码与配置、未发起任何真实调用、未改产品代码**。
  **决策的实质是把 (A) 的两个前提正面回答掉，而不是给它贴一个「暂不考虑」的标签**：
  **A1**（`domain_discovery` 的 `minimum_sources: 10` 从哪来）——今天只有两条机制能产生
  非自述来源，**两条都不通**：**声明输入**要凑 10 条就得声明 10 份输入，那会让**计数达标而
  没有任何检索发生**（正是 EC-02 WP1 点名拒绝的形态）⇒ **否决**；**工具结果**要诚实满足必须
  把 `literature.search` 接到真实 provider（`tool_providers.yaml` 已登记 `eutils-2026-08-22`）、
  把 `register_tool_evidence` **接进 run 链**、把 live 装配的**惰性工具换掉**、并新增一个**出网面**
  ——这是**一整个 cycle 的接线**，也正是 EC-02 残余 **W-7**。**A2**（11 个 phase 的会话预算）——
  E-6 实测「**phase 数 = 会话数**」⇒ 至少 **11 次真实会话**（`experiment_execution` 还要
  `TEST_PASSES`/`POLICY_COMPLIANT`），与「**live 调用取最小必要**」**直接冲突**。
  **本轮新增一条决定性实测**：`evidence.read` **未被 `policy.yaml` 授予**
  （`default_effect: DENY`）⇒ `sort_analysis_v1` 的 review phase 在**真实 policy** 下
  **连 preflight 都过不去**（此前它只在 `FakePolicyEvaluator` 下被跑过）。
  据此得到可判结论：**在册 6 份协议里没有一份同时满足「语义对真实执行体成立」与
  「合约在本轮可达」** ⇒ (B) 是**唯一可行路径**，不是偏好。
  **(B) 的规格与四条护栏已写死**（`PLAN-129` 的「WP1 定案」节）：新协议 `real_research_task_v1.yaml`
  1 个 phase、角色 `domain_researcher`、能力 `artifact.read`（**已授予**集内，与 EC-01 跑到
  `SUCCEEDED` 的那条路径同一能力集）、头部**必须**写明真实执行体语义与**受控范围**
  （单任务；不得声称多智能体并行/检索——E-6/A1 实测做不到）；合约**恰一个**声明产物 + 覆盖 ≥ 1；
  护栏包括**不得只有名字不同**、**不得删空能力绕过 preflight**、**不得空壳判据**、
  **不得改动任何既有合约使其变松**。**诚实边界**：新协议只覆盖**单任务真实研究**，
  「完整研究计划」仍不可用（需 A1 的检索接线 + E-6 的策略执行语义）⇒ **登记为残余**。

- 2026-09-21 cycle 3 **收口（WP2–WP5）**（`driver=client-goal / owner=root-agent`）：**EC-03 PASS**。
  **`RECHECK-20260921-129` = PASS_WITH_WARNINGS**（W-1…W-9）。
  **落地**：`real_research_task_v1.yaml`（1 phase / `domain_researcher` / `artifact.read` /
  头部写明真实执行体语义与受控范围）+ 新合约 `real_research_deliverable`
  （恰一个声明产物 + `EVIDENCE_COVERAGE ≥ 1`）+ 声明输入 `examples/inputs/real-research-brief-v1.json`
  + 登记面 `real-research-task`；**未削能力、未改既有合约、未动 `project.yaml`**（设计基线不重生成）。
  **判据**：`tests/api/test_real_protocol_identity.py` 把三个面钉死（**文件自己声明的 id** /
  canonical `protocol_id`（**库**优先读面）/ **被解析字节**的 sha256 + 库里的冻结正文），
  并带**常驻成对反证**（两份协议身份 / 摘要 / 正文互不相同）；**被压过**——期望值改成 demo 的 id
  ⇒ 真实那条**红**、demo 那条**仍绿** ⇒ 复原**复绿**。
  **真跑（本 cycle 仅 1 次调用）**：run `f9bef830-0ee3-4f05-8d40-ca57c1f5e643` **`SUCCEEDED`**、
  `protocol_id` 逐字为 `real_research_task_v1_0_1`、`protocol_body_digest` 与仓库文件字节
  **独立重算逐字一致**、交付物按新合约声明的名字 `analysis_report` 登记、`failures` 为空；
  默认门**如实会 skip**（离线跑该 live 模块 = `1 skipped`）。
  **一处规格外改动（如实登记）**：新 schema `schemas/real_research_deliverable_v1.schema.json`
  + `validate_bundle.py` 注册表**一行**（先例 `4156238`；已按压；校验逻辑一字未改）⇒ W-1 待人工过目。
...
- 2026-09-22 cycle 5（EC-05，`PLAN-20260922-131`）：把「默认门离线」做成**整轮结构判据**。
  判据 = `tests/egress_guard.py`（**拦**：同步 `connect`/`connect_ex` + 两个**具体**事件循环类的
  `sock_connect`，先于任何数据包抛错；**判**：`pytest_sessionfinish` 按记录把整轮判红），
  放行面只有既有的 `requires_live_llm` marker，分类复用 `endpoint_policy.destination_kind`。
  **判据上线当天就抓到一条真实的收集期出站**（`import openhands.sdk` ⇒ litellm 导入期拉
  model cost map）——默认门与 CI 在收集期一直真的出网，靠上游开关修掉源头。
  **两遍独立复检**：第一遍**证伪**了当时的头号命题（Windows Proactor 绕开同步补丁 ⇒ 异步
  出站完全逃逸），补异步面后第二遍复验通过，并指出 W-8（抽象基类桩不可达）⇒ 改装具体实现。
  **如实更正**：derive 稿的「文档地址不可路由 ⇒ 按压不会真出网」经实测**证伪**（本机代理对
  任意 IP 秒回 0.02s）⇒ 撤防类按压必然产生一次**受控出站**，已改进记录与判据注释。
  EC-05 **PASS**（`RECHECK-20260922-131` = PASS_WITH_WARNINGS，W-1/W-8 已修并复验）；
  剩余 **EC-06**（收口复检）是最后一个 EC。
- 2026-09-22 cycle 5 **CI 回灌（收口后回灌，改的是记录与 workflow，判据一行未改）**：`87208aa` 的
  M0 在 `quality-windows-latest` **红**——判据在**干净安装**下点名 **31 条** `57.150.192.193:443`
  真实出站（本机同轮 `judged 771`/`blocked 8` 全为探针）。**这是判据自己抓到的第二条真实出站**，
  且它**只在别人的机器上可见**：根因是 litellm 首次导入时 `tiktoken.get_encoding("cl100k_base")`
  下载词表，本机 `.venv` 早已热、缓存文件在 ⇒ 本地「zero」是**环境相关**的假绿。
  修法：CI 四个 Python 作业在 `uv sync` 后加一步**预热门**（判据进程之外），不碰判据、不碰放行面。
  **如实登记**：新增 `RECHECK-131` W-13（环境相关性）与本 PLAN R-6（预热门只把下载挪出判据进程，
  CI 每轮仍有一次对外请求 ⇒ 「默认 CI 完全离线」**不成立**，本轮**不声称**已离线）。
- 2026-09-22 cycle 5 **收口后 CI 全绿**（修复提交 `cf777fb`）：M0 [35639233214](https://github.com/Eswink/research-system-new/actions/runs/35639233214)
  **6/6 job success**（含 `quality-windows-latest`）、CodeQL 35639232242 3/3 success。
  **修复由判据自己验证**：同一台干净 runner 上，判据从 `87208aa` 的 `31 non-loopback destination(s)`
  变成 `judged 678` / `blocked 8`（**8 条全部是判据自证探针**），且预热门确实在 `uv sync` 之后、
  跑门之前执行（job 日志 `Run uv run --frozen --no-sync python -c "import litellm"`，见
  `scratch/ci-cf777fb-win-job.log`）⇒ 「环境准备修好、判据不放宽」这条**在 CI 上被实测，不是宣称**。
- 2026-09-22 cycle 6 **derive**（EC-06，子 PLAN `PLAN-20260922-132`，`parent_goal` 投影 ALL_PLAN）：
  本 cycle 是**最后一个 EC**——独立复检脚本（**不 import 仓库代码**）在**当前树 + 干净 checkout**
