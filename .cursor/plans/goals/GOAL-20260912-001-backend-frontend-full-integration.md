...
| EC-04 | 深水区语义（budget_adjust/预测/真 pause-resume/队列/Diff/memory policy），未落地保持诚实标注 | API + G 表一致性 | PARTIAL（budget_adjust + 成本预测 + 制品内容 Diff + pause/resume 派发协调已交付；实验队列 / memory capability policy G16 余 2 项 PENDING） |
...
按 README 的 7 步判定执行；当前续点：**cycle 8 已闭环（PLAN-048 协作式暂停派发协调交付，本地全门 + CI run #65 五 job 绿）；driver=session-goal，owner=root-agent；下一 cycle = 9（EC-04 剩余：memory capability policy G16 优先，实验队列次之；EC-05 33 路由 live-capable 与 EC-06 收口/安全扫描待排）**。
