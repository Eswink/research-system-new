...
| EC-04 | 深水区语义（budget_adjust/预测/真 pause-resume/队列/Diff/memory policy），未落地保持诚实标注 | API + G 表一致性 | PARTIAL（budget_adjust + 成本预测已交付；余项 PENDING） |
...
按 README 的 7 步判定执行；当前续点：**cycle 6 已闭环（CI run #60 达标，EC-04 第一批交付）；driver=session-goal，owner=root-agent；下一 cycle = 7（EC-04 剩余：真 pause-resume 执行协调 / 实验队列 / artifact 文件 diff / memory capability policy G16）**。
