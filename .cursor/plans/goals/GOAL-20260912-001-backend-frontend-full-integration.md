...
| EC-04 | 深水区语义（budget_adjust/预测/真 pause-resume/队列/Diff/memory policy），未落地保持诚实标注 | API + G 表一致性 | PARTIAL（budget_adjust + 成本预测 + 制品内容 Diff 已交付；真 pause-resume 协调 / 实验队列 / memory capability policy G16 余项 PENDING） |
...
按 README 的 7 步判定执行；当前续点：**cycle 7 已闭环（PLAN-047 制品内容 Diff 交付，本地全门 + stub/live e2e 全绿）；CI 终态以本轮 run 为准（状态历史登记）；driver=session-goal，owner=root-agent；下一 cycle = 8（EC-04 剩余：真 pause-resume 执行协调 / 实验队列 / memory capability policy G16；EC-05 33 路由 live-capable 全景可并行准备）**。
