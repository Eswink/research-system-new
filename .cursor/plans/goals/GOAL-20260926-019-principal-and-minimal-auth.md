...
| EC-05 | 收口复检 + 残余登记 | 两树复检 + as-is m0 **23/23** + CI 台账 + 残余逐条 | **PASS** |
...
| 3 | PLAN-20260926-194（EC-05 收口） | 收口提交（**本行由该提交写入** ⇒ 依「固定口径」其 SHA **只在回合汇报记账、不回写文件**） | **多面复检**（`scratch/goal019-ec05-multitree.py`；三面都是**真实推送过的提交**或终态工作树）：① 交付 tip `957fe05`（干净 checkout）= **1359 passed / exit=0**；② 记录 tip `75155b2`（干净 checkout）= **exit=1**（`test_reproducibility_wording.py::…test_no_affirmative_fully_reproducible_claim`）；③ 终态工作树 = **exit=0**；④ **归因唯一性**：`git diff --name-only 957fe05..75155b2` = **恰好 6 个 `.cursor/**` 记录文件**（零产品/文档/测试）；⑤ **同提交可复现**：两个独立 checkout（`D:\rs-goal019-deliv` + `D:\rs-goal019-mirror`）同数同结论（`1359 passed`）；⑥ **调用口径校准**：同提交在 `uv run --frozen --no-sync` 下 `1359 passed`，直接调 `.venv` python 则 **1345 passed / 14 failed**（`lint-imports` 不在 PATH 的**已知假红**）⇒ 跨树比较前必须统一口径。**干净树按压**：改一处口径 ⇒ 判红 `test_canonical_declaration_is_verbatim_in_all_four_docs`，sha256 `0cb60c3ee79ff14f` **逐字节还原**，复原后复跑 exit=0。**as-is 本机 m0 = `PASS: profile=m0; 23 deterministic checks`**（`M0_EXIT=0`、`PASS [` = 24、`python/tests` = **4550 passed / 20 skipped**、`FAILED`/`ERROR` 零命中；日志 `scratch/goal019-c3-m0-as-is.log`；**跑在全部收口记录写完之后**）。**记录面复跑**：`tests/architecture/python` = **186 passed**（cycle 1 为 175 ⇒ +11 = 新增判据）+ `tests/tooling` + 治理 + `DOCS-CHECK PASS: 6 deterministic checks`，全部在**记录写完之后**跑。**CI 台账**：四条历史推送逐 run 逐 job 实查（见下方台账），**红过的如实记红** | 收口提交的 run **依「固定口径」只在回合汇报记账** | **本轮抓到并修好一个真红（cycle 2 记录提交）**：`75155b2` 的 M0 = **failure**（`quality-ubuntu-latest` + `quality-windows-latest`），根因 = `tests/architecture/python/test_reproducibility_wording.py` 的 `_SCAN_ROOTS` **含 `.cursor/plans`** ⇒ `RECHECK-193` 里**裸写**被禁词表条目被判为**肯定式宣称**；**本地 m0 之所以是绿的** = 全量门跑在**记录写入之前** ⇒ **记录面从未被门覆盖**（顺序的必然，不是偶然）。修复 = 该行改为「**不得**出现的肯定式断言：… / 「完全可复现」 / …」（加否定标记 + 加引号）；本地 `tests/architecture/python` **186 passed** 验证 | **残余（原样保留 + 本轮新增）**：承继 `R-M1`（Mimosa 钩子侧未得完整结论 ⇒ **不得**宣称项目安全）/ `R-D1`（`undici` 8 条，**归属方 = 上游**）/ `R-B1` / `R-N1` / `R-F1` / `R-F2` / `W-4` / `W-5` / `W-6`（`live_run_support.py` 余量 1 行）；本 GOAL 新增 `W-10`（单一共享 token ⇒ 单一主体，**不接受**调用方自报身份）/ `W-11`（BOLA·BFLA 一个都没做）/ `W-12`（部署面未验证）/ `W-13`（前端无 token 输入面）/ `W-14`（记录面扫描面未逐一核实） | —（**终态，无下一轮输入**）。后续若要推进：`R-M1` 属 Mimosa 钩子侧、`undici` 属上游、D-12 的 (a) 面（逐调用方身份 / 对象级授权）**需另行授权** |
...
| cycle 2 记录（**红，已修**） | `75155b2`（6 个 `.cursor/**` 记录文件） | M0 [**36234092175**](https://github.com/Eswink/research-system-new/actions/runs/36234092175) = **failure**：`quality-ubuntu-latest` + `quality-windows-latest` **failure**，其余 6 job success；CodeQL [**36234091938**](https://github.com/Eswink/research-system-new/actions/runs/36234091938) **3/3 success**。**根因**（由 EC-05 多面复检归因）：记录里裸写被禁词表条目 ⇒ `test_reproducibility_wording.py` 判红（该判据的 `_SCAN_ROOTS` **含 `.cursor/plans`**）。**修法与复绿证据**：收口提交（本行下方的 run）——红**如实登记、不掩盖** |
| 收口（EC-05） | 本行所在的记录提交 | **依「固定口径」：写下某条记录的提交自身的 run 只在回合汇报记账**（不重复回写文件） |
...
| 2026-09-26 | **ACHIEVED** | cycle 3（PLAN-20260926-194）：**EC-05 = PASS** ⇒ **五个 EC 全部达成**，GOAL 置 **ACHIEVED**。**多面复检**（交付 tip `957fe05` / 记录 tip `75155b2` / 终态）抓到并修好**一个真红**：`75155b2` 的 M0 failure（`quality-ubuntu-latest` + `quality-windows-latest`），根因 = 记录里裸写被禁词表条目 ⇒ `test_reproducibility_wording.py` 判红（其 `_SCAN_ROOTS` **含 `.cursor/plans`**）；**本地 m0 之所以绿** = 全量门跑在**记录写入之前** ⇒ 记录面从未被门覆盖。**修法落成流程**：收口顺序改为「写记录 → 跑记录面判据 → 全量 m0」，并记入 `MEM-20260926-145`。**干净树按压**（改一处口径 ⇒ 判红 ⇒ sha256 逐字节还原 ⇒ 复原复绿）+ **同提交两 checkout 同结论**（`1359 passed`）。**as-is 本机 m0 = `PASS: profile=m0; 23 deterministic checks`**（`M0_EXIT=0`、`PASS [` = 24、`python/tests` = **4550 passed / 20 skipped**、`FAILED`/`ERROR` 零命中；日志 `scratch/goal019-c3-m0-as-is.log`；**跑在全部收口记录写完之后**）。残余 **九条承继 + 五条新增** 逐条登记（含归属方）。**不得宣称项目安全**（`R-M1` 未收口）。 |
