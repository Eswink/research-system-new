...
- [ ] WP-A **port + 三实现 + 契约/单测**：`LeaseHolder`/`DispatchOwnership`（port，含
...
- [ ] WP-B **控制面读面 + 文档**：DTO（`DispatchOwnershipDto` / `LeaseHolderDto` /
...
- [ ] WP-C **API 用例 + 反证 + 记录**：三态 API 用例 + 反证三跑（Edit 改回）、
  定向套件、m0、web 门、RECHECK-089 + MEM + GOAL/ALL_PLAN 回写。

## 证据

（收口时回填：提交、定向结果、m0、web 门、反证三跑、记录。）
