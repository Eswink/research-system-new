...
def scenario_cancel_then_stale_complete() -> None:
    ...
    check("cancel-then-complete: stale complete rejected",
...
def scenario_cancel_then_recover() -> None:
    ...
    st = _state(task.id.value)
    check("cancel-then-recover: no resurrection",
...
def scenario_concurrent_cancel_and_complete() -> None:
    ...
    for iteration in range(3):
        ...
        check(f"iter{iteration} cancel-vs-complete race: legal outcome",
              legal and not_both and ok, f"state={st}")
